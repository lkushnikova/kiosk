...
<div class="news white_news">
    ...
    if ( count($doctor_result) ) {
        foreach($doctor_result as $doctor_row) {
            ...
            echo '<hr>';
            echo '<p class="big_p">Дата приема: '.date_create($date)->Format('d.m.Y').'</p>';
            echo '<p class="big_p">Время приема: '.$time.'</p>';
        }
        echo '<div class=" for_but for_confirm"><a href="patient_info.php?id='.$row['dct_id'].'&date_for_time='.$formatted_date.'&time='.$time.'">Записаться</a></div>';
